...
const foundationKeys = ['choose','setup','prompt','privacy'];
const pathKeys = ['chat','work','codex'];
const chatPracticeKeys = ['files','search','projects'];
const workPracticeKeys = ['compare','documents','review'];
const codexPracticeKeys = ['plan','debug','publish'];
const codexBasicsKeys = ['workspace','folders','run'];
const codexMaintenanceKeys = ['modify','changes','backup'];
const chatCompletionKeys = ['followup','clues','images','teaching'];
const keys = [...foundationKeys,'chat','followup','clues','files','images','search','projects','teaching','work','environment','brief','compare','documents','redirect','review','reuse','codex','workspace','folders','plan','run','modify','debug','changes','backup','publish'];
const publishedByGroup = {start:foundationKeys,chat:{0:'chat',1:'followup',2:'clues',3:'files',4:'images',5:'search',6:'projects',7:'teaching'},work:{0:'work',1:'environment',2:'brief',3:'compare',4:'documents',5:'redirect',6:'review',7:'reuse'},codex:{0:'codex',1:'workspace',2:'folders',3:'plan',4:'run',5:'modify',6:'debug',7:'changes',8:'backup',9:'publish'}};
...
const courses = [
 {key:'start',title:'공통 입문 · 처음 오셨나요?',items:foundationKeys.map(k=>lessons[k].title)},
...
function home() {
 ...
 ${progress.last ? `<div class="progress-banner"><span><strong>다시 오셨네요.</strong> 지난 학습부터 이어갈까요?</span><div class="progress-track" aria-label="전체 학습 ${progress.completed.length}개 완료"><span style="width:${progress.completed.length/keys.length*100}%"></span></div><span>${progress.completed.length}/${keys.length} 완료</span><a href="#/lesson/${progress.last}">${names[progress.last]} 이어보기 →</a></div>`:''}
...
function chatCompletionRoute() {
 return '<section class="course-group foundation-route"><h2>질문에서 업무 활용까지 · 채팅 4편</h2><p class="foundation-intro">후속 요청과 이미지 질문을 익히고 교사 업무에 적용합니다. 채팅 과정 8편이 모두 공개되었습니다.</p>'+chatCompletionKeys.map((key,i)=>`<a class="course-row published" href="#/lesson/${key}"><span class="order">0${i+1}</span><span>${esc(lessons[key].title)}</span><span class="badge ready">${progress.completed.includes(key)?'✓ 완료':'약 '+lessons[key].minutes+'분 →'}</span></a>`).join('')+'</section>';
}
function chatPracticeRoute() {
 return '<section class="course-group foundation-route"><h2>채팅을 업무에 활용하기 · 실전 3편</h2><p class="foundation-intro">자료를 읽고, 근거를 확인하고, 다음 작업에 이어 쓰는 연습입니다.</p>'+chatPracticeKeys.map((key,i)=>`<a class="course-row published" href="#/lesson/${key}"><span class="order">0${i+1}</span><span>${esc(lessons[key].title)}</span><span class="badge ready">${progress.completed.includes(key)?'✓ 완료':'약 '+lessons[key].minutes+'분 →'}</span></a>`).join('')+'</section>';
}
function workCompletionRoute() {
 return '<section class="course-group foundation-route"><h2>작업을 맡기고 이어가기 · Work 4편</h2><p class="foundation-intro">작업 환경과 완료 조건을 정하고, 방향을 바꾸고, 다음 일에 재사용합니다. Work 과정 8편이 모두 공개되었습니다.</p>'+['environment','brief','redirect','reuse'].map((key,i)=>`<a class="course-row published" href="#/lesson/${key}"><span class="order">0${i+1}</span><span>${esc(lessons[key].title)}</span><span class="badge ready">${progress.completed.includes(key)?'✓ 완료':'약 '+lessons[key].minutes+'분 →'}</span></a>`).join('')+'</section>';
}
function workPracticeRoute() {
 return '<section class="course-group foundation-route"><h2>Work로 결과물 완성하기 · 실전 3편</h2><p class="foundation-intro">비교할 기준을 정하고, 문서와 발표자료를 만든 뒤, 원본과 결과물을 대조합니다.</p>'+workPracticeKeys.map((key,i)=>`<a class="course-row published" href="#/lesson/${key}"><span class="order">0${i+1}</span><span>${esc(lessons[key].title)}</span><span class="badge ready">${progress.completed.includes(key)?'✓ 완료':'약 '+lessons[key].minutes+'분 →'}</span></a>`).join('')+'</section>';
}
function codexMaintenanceRoute() {
 return '<section class="course-group foundation-route"><h2>만든 도구 오래 쓰기 · 수정과 복원 3편</h2><p class="foundation-intro">기존 기능을 지키며 수정하고, 변경을 검토하고, 보관본으로 돌아가는 방법을 익힙니다. Codex 과정 10편이 모두 공개되었습니다.</p>'+codexMaintenanceKeys.map((key,i)=>`<a class="course-row published" href="#/lesson/${key}"><span class="order">0${i+1}</span><span>${esc(lessons[key].title)}</span><span class="badge ready">${progress.completed.includes(key)?'✓ 완료':'약 '+lessons[key].minutes+'분 →'}</span></a>`).join('')+'</section>';
}
function codexBasicsRoute() {
 return '<section class="course-group foundation-route"><h2>Codex를 열었다면 · 준비 3편</h2><p class="foundation-intro">작업 위치와 파일을 확인한 뒤, 브라우저에서 직접 실행해 봅니다.</p>'+codexBasicsKeys.map((key,i)=>`<a class="course-row published" href="#/lesson/${key}"><span class="order">0${i+1}</span><span>${esc(lessons[key].title)}</span><span class="badge ready">${progress.completed.includes(key)?'✓ 완료':'약 '+lessons[key].minutes+'분 →'}</span></a>`).join('')+'</section>';
}
function codexPracticeRoute() {
 return '<section class="course-group foundation-route"><h2>내 도구를 발전시키기 · Codex 실전 3편</h2><p class="foundation-intro">수업 운영 보드를 기획하고, 오류를 해결하고, 인터넷에 공개하는 흐름을 연습합니다.</p>'+codexPracticeKeys.map((key,i)=>`<a class="course-row published" href="#/lesson/${key}"><span class="order">0${i+1}</span><span>${esc(lessons[key].title)}</span><span class="badge ready">${progress.completed.includes(key)?'✓ 완료':'약 '+lessons[key].minutes+'분 →'}</span></a>`).join('')+'</section>';
}
function startPage() {
 return `<div class="container page-bottom">${heading('YOUR FIRST STEP','15분, 첫 성공을 만들어 볼까요?','처음 5분은 차이를 이해하고, 다음 10분은 안내문 한 편을 다듬어 봅니다.')}<div class="intro-grid"><div class="intro-card"><span class="icon-tile">${icon('chat')}</span><h2>채팅은 대화하며</h2><p>생각을 정리하고 글을 다듬어요. 답을 보고 다시 부탁하면서 원하는 방향을 찾습니다.</p></div><div class="intro-card"><span class="icon-tile">${icon('work')}</span><h2>Work는 목표를 맡겨</h2><p>자료와 완료 조건을 주고 검토할 결과물을 받아요. 문서 작업과 자료 분석부터 시작해 봅니다.</p></div><div class="intro-card"><span class="icon-tile">${icon('code')}</span><h2>Codex는 만들고 고쳐</h2><p>작동하는 도구를 만들고 발전시켜요. 파일과 실행 결과를 확인하는 경험을 익힙니다.</p></div></div><div class="info-note">세 경로는 배우기 쉬운 출발점이에요. 기능은 서로 겹칩니다. Work에서도 프로그램 관련 일을 할 수 있고, Codex로 문서를 만들 수도 있어요. <a href="https://learn.chatgpt.com/docs/use-chatgpt" target="_blank" rel="noopener noreferrer">공식 설명 확인 ↗</a></div>${foundationRoute()}<h2>바로 해보고 싶다면 · 첫날에는 여기까지만</h2><ol class="step-route"><li><span class="step-num">01</span><div><h3>내가 원하는 결과를 골라요 <span class="badge">2분</span></h3><p>오늘은 ‘학부모가 읽기 편한 안내문’을 목표로 정합니다.</p></div></li><li><span class="step-num">02</span><div><h3>ChatGPT를 열고 준비해요 <span class="badge">3분</span></h3><p>본인 계정으로 로그인하고 새 채팅을 여세요. 학생 개인정보는 필요하지 않아요.</p></div><a class="button small" href="https://chatgpt.com/" target="_blank" rel="noopener noreferrer">ChatGPT 열기 ↗</a></li><li><span class="step-num">03</span><div><h3>초안을 받고 한 번 고쳐요 <span class="badge">10분</span></h3><p>예시를 복사하고, 사실을 확인하고, 길이를 줄이는 실습입니다.</p></div><a class="button primary small" href="#/lesson/chat">안내문 실습 ${icon('arrow')}</a></li></ol><aside class="teacher-note"><span class="avatar">G</span><div><strong>시작 전 확인</strong><p>이 사이트는 학습 안내서입니다. AI가 실행되는 곳은 ChatGPT 또는 Codex예요. 실습 가능한 기능은 계정과 사용 환경에 따라 달라질 수 있습니다. 기본 안내는 Windows 기준이며 Mac에서는 메뉴가 다를 수 있어요.</p></div></aside>${chatPracticeRoute()}${chatCompletionRoute()}${workPracticeRoute()}${workCompletionRoute()}${codexBasicsRoute()}${codexPracticeRoute()}${codexMaintenanceRoute()}<h2>다른 도구로 넓혀 보기</h2><div class="tool-grid"><a href="#/lesson/work" class="tool-link">${icon('work')}<div><h3>자료로 결과물 만들기</h3><p>안내문에서 배포용 안내자료로 넓혀 보세요.</p></div><span class="arrow">→</span></a><a href="#/lesson/codex" class="tool-link">${icon('code')}<div><h3>나의 수업 도구 만들기</h3><p>수업 순서와 자료를 운영 보드로 모아 보세요.</p></div><span class="arrow">→</span></a></div></div>`;
}
function coursesPage() {
 return `<div class="container page-bottom">${heading('LEARNING MAP','내 속도로 따라가는 학습 지도','공통 입문부터 차근차근 배우거나, 지금 필요한 대표 실습으로 바로 이동하세요.')}<div class="info-note">현재 <strong>공통 입문 4편 + 채팅 8편 + Work 8편 + Codex 10편, 총 30편</strong>을 이용할 수 있습니다. 계획한 30개 주제가 모두 공개되었습니다.</div><div class="course-tabs" role="group" aria-label="학습 경로 필터">${[['all','전체'],['start','처음 시작'],...pathKeys.map(k=>[k,names[k]])].map(([k,n])=>`<button data-filter="${k}" class="${k==='all'?'active':''}" aria-pressed="${k==='all'}">${n}</button>`).join('')}</div>${courses.map(g=>`<section class="course-group" data-group="${g.key}"><h2>${g.title} <span class="badge">${g.items.length}개 주제</span></h2>${g.items.map((t,i)=>{const lessonKey=publishedByGroup[g.key]?.[i];const ready=!!lessonKey;return `<${ready?'a':'div'} ${ready?`href="#/lesson/${lessonKey}"`:''} class="course-row ${ready?'published':'planned'}"><span class="order">${String(i+1).padStart(2,'0')}</span><span>${t}</span><span class="badge ${ready?'ready':''}">${ready?(g.key==='start'?'학습하기 →':'실습하기 →'):'준비 예정'}</span></${ready?'a':'div'}>`;}).join('')}</section>`).join('')}</div>`;
}
const resources = [
 {title:'Work 작업설계표',description:'작업 환경·자료·완료 조건·방향 변경·재사용을 기록합니다.',file:'2026-09-22-Work-작업설계표.md',meta:'Markdown · Work 실습'},
 {title:'채팅 활용 기록표',description:'후속 요청·네 가지 단서·이미지 대조·수업 적용을 기록합니다.',file:'2026-09-22-채팅활용-기록표.md',meta:'Markdown · 채팅 과정 실습'},
 {title:'사진 질문용 가상 일정',description:'HTML을 브라우저로 열고 일정 카드만 캡처해 이미지 질문에 사용하세요.',file:'2026-09-22-사진질문-연습화면.html',meta:'HTML · 가상 일정 화면'},
 {title:'Codex 수정·복원 기록표',description:'작은 변경의 영향과 코드·수업 데이터 복원 결과를 따로 기록합니다.',file:'2026-09-21-Codex-수정복원기록표.md',meta:'Markdown · 변경 검토와 복원 확인'},
 {title:'Codex 준비 점검표',description:'작업 폴더·파일·브라우저 저장 자료를 구분하고 실행 결과를 기록합니다.',file:'2026-09-21-Codex-준비점검표.md',meta:'Markdown · 시작 위치와 실행 확인'},
 {title:'Codex 실습 기록표',description:'기획의 완료 조건, 오류 재현 기록, 공개 전후 점검을 한 장의 기록으로 이어갑니다.',file:'2026-09-21-Codex-실습기록표.md',meta:'Markdown · 기획·수정·배포 기록'},
 {title:'탐구 준비물 가상 비교자료',description:'세 가지 가상 공급안의 비용과 미확정 조건을 비교합니다. 실제 견적이나 구매 추천 자료가 아닙니다.',file:'2026-09-21-탐구준비물-비교자료.csv',meta:'CSV · Work 비교·분석 실습'},
 {title:'공개수업 검토용 초안',description:'연습을 위해 오류를 넣은 초안입니다. 계획·메모와 대조하고 고쳐 보세요. 실제 배포 금지.',file:'2026-09-21-공개수업-검토용초안.txt',meta:'TXT · 의도적 오류가 있는 가상 자료'},
 {title:'Work 결과 검토표',description:'분석의 계산·미정 조건과 문서의 원본 근거를 직접 확인하는 점검표입니다.',file:'2026-09-21-Work-결과검토표.md',meta:'Markdown · 메모장으로 열 수 있어요'},
 {title:'공개수업 가상 계획서',description:'일정·장소·수업 흐름이 정리된 자료예요. 파일 질문·프로젝트·Work 실습에서 준비 메모와 함께 사용합니다.',file:'2026-09-20-공개수업-계획.txt',meta:'TXT · 가상 자료 · 3학년 과학'},
...
function lessonPage(key) {
 ...
 return `<div class="lesson-shell"><aside class="lesson-sidebar"><p class="sidebar-title">작은 성공부터, 차근차근</p><a class="sidebar-link" href="#/start">${icon('leaf')} 처음 시작하기</a>${keys.map(k=>`<a class="sidebar-link ${k===key?'active':''}" href="#/lesson/${k}" ${k===key?'aria-current="page"':''}>${icon(foundationKeys.includes(k)?'book':k==='codex'?'code':k)}${names[k]} ${progress.completed.includes(k)?'✓':''}</a>`).join('')}<a class="sidebar-link" href="#/resources">${icon('folder')} 실습 자료</a><a class="sidebar-link" href="#/help">${icon('help')} 문제 해결</a><div class="sidebar-note">전체 학습 ${progress.completed.length}/${keys.length} 완료<br>학습 기록은 이 브라우저에<br>저장됩니다.</div></aside><article class="lesson-article"><div class="breadcrumb"><a href="#/">홈</a><span>/</span><a href="#/courses">학습 과정</a><span>/</span><span>${names[key]}</span></div><div class="eyebrow">${esc(l.eyebrow)}</div><h1 class="lesson-title">${esc(l.title)}</h1><p class="lesson-summary">${esc(l.summary)}</p><div class="lesson-meta"><span class="badge">${key==='codex'?'첫 프로젝트':'초보 필수'}</span><span>약 ${l.minutes}분</span><span>Windows 기본 안내</span><span>${esc(l.checkedDate || '2026-09-20')} 확인</span></div><div class="outcome"><strong>오늘의 완성 목표</strong><p>${esc(l.outcome)}</p></div>
