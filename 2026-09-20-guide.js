...
const names = {choose:'도구 선택',setup:'시작 준비',prompt:'요청과 후속 질문',privacy:'자료와 개인정보',chat:'ChatGPT 채팅',work:'ChatGPT Work',codex:'Codex'};
const foundationKeys = ['choose','setup','prompt','privacy'];
const pathKeys = ['chat','work','codex'];
const keys = [...foundationKeys,...pathKeys];
...
const faqItems = [
 ...
 {id:'progress',group:'이 가이드 이용하기',question:'완료 표시와 이어보기는 어디에 저장되나요?',answer:'이 가이드를 여는 현재 브라우저에 저장됩니다. 회원가입이나 서버 전송은 없습니다. 다른 기기·브라우저에서는 이어지지 않으며, 브라우저 데이터를 지우면 초기화될 수 있습니다. 공통 입문 4편과 대표 실습 3편, 총 7편에 완료 표시가 제공됩니다.',tags:'진도 기록 학습 완료 개인정보'},
 {id:'planned',group:'이 가이드 이용하기',question:'학습 지도에 준비 예정이라고 표시돼요.',answer:'지금은 공통 입문 4편과 세 경로의 대표 실습 3편을 공개한 버전입니다. 학습 지도에는 앞으로 확장할 목차도 함께 표시했습니다. ‘학습하기’ 또는 ‘실습하기’가 붙은 페이지를 이용해 주세요.',tags:'페이지 준비 개발 목차'}
];
const courses = [
...
function coursesPage() {
 return `<div class="container page-bottom">${heading('LEARNING MAP','내 속도로 따라가는 학습 지도','공통 입문부터 차근차근 배우거나, 지금 필요한 대표 실습으로 바로 이동하세요.')}<div class="info-note">현재 <strong>공통 입문 4편 + 대표 실습 3편, 총 7편</strong>을 이용할 수 있습니다. 아래 30개 주제 중 나머지 23개는 준비 예정입니다.</div><div class="course-tabs" role="group" aria-label="학습 경로 필터">${[['all','전체'],['start','처음 시작'],...pathKeys.map(k=>[k,names[k]])].map(([k,n])=>`<button data-filter="${k}" class="${k==='all'?'active':''}" aria-pressed="${k==='all'}">${n}</button>`).join('')}</div>${courses.map(g=>`<section class="course-group" data-group="${g.key}"><h2>${g.title} <span class="badge">${g.items.length}개 주제</span></h2>${g.items.map((t,i)=>{const lessonKey=g.key==='start'?foundationKeys[i]:(pathKeys.includes(g.key)&&i===0?g.key:null);const ready=!!lessonKey;return `<${ready?'a':'div'} ${ready?`href="#/lesson/${lessonKey}"`:''} class="course-row ${ready?'published':'planned'}"><span class="order">${String(i+1).padStart(2,'0')}</span><span>${t}</span><span class="badge ${ready?'ready':''}">${ready?(g.key==='start'?'학습하기 →':'실습하기 →'):'준비 예정'}</span></${ready?'a':'div'}>`;}).join('')}</section>`).join('')}</div>`;
}
const resources = [
