...
const files = ['2026-09-20-lessons.js', '2026-09-20-foundations.js', '2026-09-21-chat-practice.js', '2026-09-21-work-practice.js', '2026-09-21-codex-practice.js', '2026-09-20-guide.js'];
...
test('Three, seven, ten and thirteen-lesson progress survive sixteen-lesson upgrade', () => {
  for (const saved of [{completed:['chat','work'],last:'work'}, {completed:['chat','work','privacy','prompt'],last:'privacy'}, {completed:['choose','setup','prompt','privacy','chat','files','search','projects','work','codex'],last:'projects'}, {completed:['choose','setup','prompt','privacy','chat','files','search','projects','work','compare','documents','review','codex'],last:'review'}]) {
    const app=setup(saved);
    assert.ok(app.html().includes(saved.completed.length+'/16 완료'));
...
test('Learning map exposes exactly sixteen usable lessons and 14 prepared topics', () => {
  const app=setup(undefined,'#/courses');
  assert.equal((app.html().match(/class="course-row published"/g)||[]).length,16);
  assert.equal((app.html().match(/class="course-row planned"/g)||[]).length,14);
  for(const key of ['choose','setup','prompt','privacy','chat','files','search','projects','work','compare','documents','review','codex','plan','debug','publish']) assert.ok(app.html().includes('href="#/lesson/'+key+'"'));
...
test('New completion persists through restart, cancellation persists, old completions remain', async () => {
  let app=setup({completed:['chat','work','privacy','prompt'],last:'privacy'},'#/lesson/publish');
  ...
  assert.match(app.html(),/전체 학습 5\/16 완료/);
  assert.match(app.html(),/✓ 학습 완료 · 취소/);
  await app.complete();
  app=setup(app.saved());
  assert.deepEqual(app.saved().completed,['chat','work','privacy','prompt']);
  assert.match(app.html(),/4\/16 완료/);
});
test('Every new title and a text-only body fragment are searchable', () => {
  const app=setup();
  for(const key of ['choose','setup','prompt','privacy','files','search','projects','compare','documents','review','plan','debug','publish']) {
...
test('Cross-tab completion event updates visible lesson count and button', async () => {
  ...
  assert.match(app.sidebar(),/전체 학습 2\/16 완료/);
  const button=await app.complete();
...
test('Unknown and duplicate legacy completion entries cannot inflate progress', () => {
  ...
  assert.match(app.html(),/전체 학습 2\/16 완료/);
...
test('Practice lessons render with correct links from Chat through Work and the full Codex practice sequence', () => {
  const app=setup();
  const sequence=['privacy','chat','files','search','projects','work','compare','documents','review','codex','plan','debug','publish'];
...
test('Codex learning map publishes overview, planning, debugging and publication', () => {
  const app=setup(undefined,'#/courses');
  ...
  rows.forEach((row,i)=> {
    const key={0:'codex',3:'plan',6:'debug',9:'publish'}[i];
    if(key) assert.ok(row.includes('href="#/lesson/'+key+'"'));
    else assert.match(row,/course-row planned/);
  });
});
test('Published entrypoint loads new lesson script before guide and rendered local downloads exist', () => {
  ...
  for(const resource of ['2026-09-21-탐구준비물-비교자료.csv','2026-09-21-공개수업-검토용초안.txt','2026-09-21-Work-결과검토표.md','2026-09-21-Codex-실습기록표.md']) {
  ...
  for(const route of ['#/resources','#/lesson/files','#/lesson/search','#/lesson/projects','#/lesson/compare','#/lesson/documents','#/lesson/review','#/lesson/plan','#/lesson/debug','#/lesson/publish']) {
    app.route(route);
